...
class Vender extends Component {
    ...
    if (this.FormLayout.validate().isValid == true) {
      Swal.fire({
        ...
        if (res.value) {
          ...
          data.append("name", this.state.jFormData.name);
          data.append("country", this.state.jFormData.country);
          data.append("logo", this.state.File);

          console.log("adasdasd", data);
          var config = {
            method: "post",
            url: `${APIURl.URL}vendor/add`,
            headers: {
              Authorization:
  ...
  renderGridCell(cellData) {
    ...
    return (
      <div>
        <img style={{width: "100px",height: "100px"}} src={cellData.value}></img>
      </div>
    );
  }
